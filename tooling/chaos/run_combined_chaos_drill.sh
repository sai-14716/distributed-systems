...
TARGETS="${TARGETS:-$(python3 tooling/cluster_helper.py get_all_urls nodes | tr ' ' ',')}"
...
find_leader() {
  for _ in {1..30}; do
    for n in $(python3 tooling/cluster_helper.py get_all nodes); do
      url=$(python3 tooling/cluster_helper.py get_url nodes "$n")
      state="$(curl -s --max-time 1 "${url}/state" || true)"
...
log "bringing up cluster (local components only)"
docker compose up -d --build >"$DRILL_DIR/cluster_up.log" 2>&1
