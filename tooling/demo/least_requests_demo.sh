...
( curl -s --max-time 6 -D "$hdr_a" -o /dev/null "$LB_URL/chat" -H "X-Session-ID: demo-A" -H "X-Work-Ms: 5000" || true ) &
pid=$!

sleep 0.2

echo "Sending short request B while A is in-flight (least-req should prefer the backend with fewer active requests)..."
curl -s --max-time 1 -D "$hdr_b" -o /dev/null "$LB_URL/chat" -H "X-Session-ID: demo-B" -H "X-Work-Ms: 50" || true
