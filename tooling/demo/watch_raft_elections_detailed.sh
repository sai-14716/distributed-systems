...
while true; do
  ...
  # Get both node states in parallel
  NODE1_STATE=$(curl -s http://localhost:19091/state 2>/dev/null || echo '{"error":"unreachable"}')
  NODE2_STATE=$(curl -s http://localhost:19092/state 2>/dev/null || echo '{"error":"unreachable"}')
